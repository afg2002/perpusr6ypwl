    ...
    if (mysqli_num_rows($result) > 0) {
      ...
      while ($row = mysqli_fetch_assoc($result)) {
        ...
        if ($selisih_hari > 0) {
          ...
          echo '<td><button onclick="konfirmasiKembalikan(' . $row['id_buku'] . ', ' . $denda . ')">Kembalikan</button> (Denda: Rp' . $denda . ')</td>';
        } else {
          echo '<td>' . $row['tanggal_kembali'] . '</td>';
          echo '<td><button onclick="konfirmasiKembalikan(' . $row['id_buku'] . ', 0)">Kembalikan</button></td>';
        }
        echo '</tr>';
      }
